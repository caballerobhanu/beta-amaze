...
export default function EditTournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [gamesSnap, tourneysSnap, docSnap] = await Promise.all([
        ...
        if (docSnap.exists()) {
          ...
          setFormData({
            name: data.name || "", shortName: data.shortName || "", slug: data.slug || "",
            gameId: data.gameId || "", status: data.status || "upcoming",
            series: data.series || "", parentTournamentId: data.parentTournamentId || "",
            tier: data.tier || "S-Tier", eventType: data.eventType || "Online",
            locations: data.locations || (data.location ? [{ label: "Main", venue: data.location }] : []),
            prizePools: data.prizePools || (data.prizePool ? [{ currency: "USD", amount: data.prizePool }] : []),
            startDate: data.startDate || "", endDate: data.endDate || "",
            coverImage: data.coverImage || "",
            thumbnailLight: data.thumbnailLight || data.thumbnail || "",
            thumbnailDark: data.thumbnailDark || "",
            organizers: data.organizers || [], publishers: data.publishers || [], sponsors: data.sponsors || [],
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const docData: any = { ...formData, updatedAt: serverTimestamp() };
