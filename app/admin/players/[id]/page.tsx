...
export default function EditPlayerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [gamesSnap, teamsSnap, docSnap] = await Promise.all([
        ...
        if (docSnap.exists()) {
          ...
          setFormData({
            ...
            photoUrl: (data.photos && data.photos.length > 0) ? data.photos[0] : "",
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "players", id), docData);
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold tracking-tight">Edit Player</h1>
        <p className="text-neutral-600 dark:text-neutral-400">Update player profile.</p>
      </div>
